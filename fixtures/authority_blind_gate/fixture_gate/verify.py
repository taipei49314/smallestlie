"""
Isolation fixture: authority_blind_gate

Intentionally ignores approver allowlist (AUTH-001 surface).
Still binds evidence revision and protected path.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    # WEAKNESS: authority ignored
    approval = _load_json(root / "authority" / "approval.json")
    _ = approval.get("approver")
...
def _emit(root: Path, accepted: bool, status: str, reasons: list, tests=None, revision=None) -> int:
    ...
    report = {
        ...
        "tests": tests or [],
        "revision": revision,
        "notes": ["authority_blind_gate ignores approver allowlist"],
